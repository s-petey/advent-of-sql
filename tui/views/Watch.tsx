    ...
    if (key.name === "t") {
      if (currentYear !== todayYear) {
        if (availableYears.includes(todayYear)) {
          ...
          setWarning(`Switched to ${todayYear}. Press t again to run today.`);
        } else {
          setWarning(`No files exist for ${todayYear}. Create day ${todayDay} first!`);
        }
        return;
      }
      ...
      if (!todayExists) {
        setWarning(`Day ${todayDay} doesn't exist for ${todayYear}. Create it first!`);
        return;
      }
    ...
    switch (key.name) {
      ...
      case "left":
        setSelectedIndex((prev) => {
          ...
          const lastCol = Math.floor((days.length - 1) / ROWS_PER_COLUMN);
      ...
      case "up":
        setSelectedIndex((prev) => {
          ...
          return newIndex < days.length && newIndex >= 0 ? newIndex : prev;
        });
        break;
      case "return":
      case "w": {
  ...
  return (
    <>
      ...
      >
        ...
        {loading ? (
          ...
          >
            <text style={{ fg: theme["Autumn Ember"] }}>
              No days available for {currentYear}. Create one first!
            </text>
          </box>
        ) : (
          <box
            style={{
              ...
              gap: 4,
              justifyContent: "center",
              borderColor: isDaysFocused ? theme["Toasted Almond"] : "transparent",
              padding: 1,
            }}
            ...
            {columns.map((column, colIdx) => (
              ...
              >
                {column.map((option, rowIdx) => {
                  const globalIndex = colIdx * ROWS_PER_COLUMN + rowIdx;
                  const isSelected = globalIndex === selectedIndex && isDaysFocused;
                  return (
                    <text
                      key={`${option.year}-${option.day}`}
                      style={{
                        fg: isSelected ? theme["Toasted Almond"] : theme.Silver,
                        ...(isSelected
                          ? {
                              attributes: TextAttributes.BOLD,
                            }
                    ...
                    >
                      {isSelected ? "> " : "  "}
                      Day {String(option.day).padStart(2, "0")}
                    </text>
                  );
                })}
              </box>
        ...
        <box
          style={{
            ...
            gap: 4,
            marginTop: 2,
            borderColor: isYearFocused ? theme["Toasted Almond"] : "transparent",
            padding: 1,
          }}
        ...
        >
          <text
            style={{
              fg: hasPreviousYear ? theme.Silver : theme["Charcoal Blue"],
            }}
          >
            &#x2190; {hasPreviousYear ? availableYears[currentYearIndex + 1] : "----"}
          </text>
          <text
            style={{
              fg: isYearFocused ? theme["Toasted Almond"] : theme["Autumn Ember"],
              ...(isYearFocused ? { attributes: TextAttributes.BOLD } : {}),
            }}
          >
            {currentYear}
          </text>
          <text
            style={{
              fg: hasNextYear ? theme.Silver : theme["Charcoal Blue"],
            }}
          >
            {hasNextYear ? availableYears[currentYearIndex - 1] : "----"} &#x2192;
          </text>
        </box>
      ...
      <Footer>
        ...
        >
          <strong style={{ fg: theme["Autumn Ember"] }}>Tab</strong> to switch between days/year.{" "}
          <strong style={{ fg: theme["Autumn Ember"] }}>h j k l</strong> or{" "}
          <strong style={{ fg: theme["Autumn Ember"] }}>Arrow keys</strong> to navigate.{" "}
          <strong style={{ fg: theme["Autumn Ember"] }}>t</strong> run today.{" "}
          <strong style={{ fg: theme["Autumn Ember"] }}>Enter | w</strong> to start watch.{" "}
