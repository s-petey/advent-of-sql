...
function getDayTouple(dateToUse: Date) {
  const [one = 0, two = 0] = dateToUse.getDate().toString().split("").map(Number);
...
export function NewView({ setView }: ContentProps) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const errorFields = isValidDayYear(Number(year.join("")), Number(day.join("")));
        ...
        >
          <Button
            ...
            style={{
              ...
              width: 20,
              height: 3,
              borderColor: inputFocus === "day" ? theme["Toasted Almond"] : theme.Silver,
            }}
          >
            {day.map((char, idx) => (
              <text key={`${idx}-${char}`} fg={dayIdx === idx ? theme["Autumn Ember"] : "white"}>
                {char}
              </text>
            ))}
          </Button>
          ...
          <Button
            ...
            style={{
              ...
              width: 40,
              height: 3,
              borderColor: inputFocus === "year" ? theme["Toasted Almond"] : theme.Silver,
            }}
          >
            {year.map((char, idx) => (
              <text key={`${idx}-${char}`} fg={yearIdx === idx ? theme["Autumn Ember"] : "white"}>
                {char}
              </text>
            ))}
          </Button>
