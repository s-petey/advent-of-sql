...
const success = (text: string) => renderColor(Doc.annotate(Doc.text(text), Ansi.green));
const error = (text: string) => renderColor(Doc.annotate(Doc.text(text), Ansi.red));
const info = (text: string) => renderColor(Doc.annotate(Doc.text(text), Ansi.cyan));
const warning = (text: string) => renderColor(Doc.annotate(Doc.text(text), Ansi.yellow));
const highlight = (text: string) =>
  renderColor(Doc.annotate(Doc.text(text), Ansi.brightColor(Color.magenta)));
const bold = (text: string) => renderColor(Doc.annotate(Doc.text(text), Ansi.bold));
...
const yearArg = Args.integer({ name: "year" }).pipe(Args.withDefault(currentYear));

// "new" command - creates new day or year
const newCommand = Command.make("new", { day: dayArg, year: yearArg }, ({ day, year }) => {
...
const runCommand = Command.make("run", { day: dayArg, year: yearArg }, ({ year, day }) => {
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const cwd = process.cwd();
    ...
    yield* Effect.tryPromise({
      ...
      catch: (error) => new Error(`Failed to run ${year}/${day}.ts: ${error}`),
    });
  });
});

// "watch" command - watches and re-runs on file changes
const watchCommand = Command.make("watch", { day: dayArg, year: yearArg }, ({ year, day }) => {
  return Effect.gen(function* () {
    ...
    const showHelp = Effect.gen(function* () {
      ...
      yield* Effect.log(highlight("  R    ") + "  - Reset database and re-run");
    ...
    const resetDB = Effect.gen(function* () {
      ...
      if (Either.isLeft(pgConnected)) {
        ...
        yield* Effect.log(
          info("Please start PostgreSQL and ensure it's running on localhost:5432"),
        );
        yield* Effect.log(info("Default credentials: postgres/postgres"));
        yield* Effect.log(info("Press R again to retry after starting the database"));
        yield* Effect.log(separator("=", 50) + "\n");
        return;
      }
      ...
      if (sqlFileExists) {
        ...
        yield* Effect.log(success(`✓ Database reset with: ${year}/${day}.sql`));
      } else {
        yield* Effect.log(info(`No SQL reset file found: ${year}/${day}.sql`));
        yield* Effect.log(info(`Run 'new ${day} ${year}' to create it first.`));
      }
    ...
    const runFile = Effect.gen(function* () {
      ...
      if (Either.isLeft(result)) {
        ...
        yield* Effect.log(info("   Fix the error and save the file to resume auto-running"));
        yield* Effect.log(info("   Or press:"));
        yield* Effect.log(highlight("     Enter") + "  - Manually re-run the current file");
        yield* Effect.log(highlight("     R    ") + "  - Reset database and re-run");
        yield* Effect.log(separator("=", 50) + "\n");
        return;
      }
    ...
    const combinedStream = Stream.merge(
      fileChangeStream.pipe(Stream.map(() => ({ type: "fileChange" as const }))),
      keyboardStream.pipe(Stream.map((key) => ({ type: "keyboard" as const, key }))),
    );
      ...
      Stream.runForEach((event) => {
        if (event.type === "fileChange") {
          ...
          if (hasError) {
            hasError = false;
            return Effect.gen(function* () {
              yield* Effect.log(success("✓ File saved, resuming watch mode..."));
              yield* runFile;
            });
          }
          return runFile;
